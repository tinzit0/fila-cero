...
const RPC_START_BOOKING=FCDB.startBookingRpc||'fila_cero_start_booking';
const RPC_PAYMENT_STATUS=FCDB.paymentStatusRpc||'fila_cero_payment_status';
const EDGE_CREATE_PAYMENT=window.FC_CONFIG?.edge?.createPayment||'fila-cero-create-payment';
...
function normalizeSlot(row,business=null){const b=business||row.business||null;return {id:row.id,businessId:row.business_id,provider:b?.name||'Centro profesional',service:row.service,category:row.category,city:row.city,sector:row.sector||'',date:row.slot_date,time:String(row.start_time||'').slice(0,5),normalPrice:Number(row.normal_price||0),price:Number(row.fila_price||0),duration:Number(row.duration_minutes||30),address:row.address,status:row.status,createdAt:row.created_at,requiresDeposit:row.requires_deposit===true,depositAmount:Number(row.deposit_amount||0),latitude:Number(b?.latitude)||null,longitude:Number(b?.longitude)||null,business:b||null}}
function humanError(err){const msg=String(err?.message||err||'Error inesperado');const low=msg.toLowerCase();if(msg.includes('SLOT_UNAVAILABLE')||msg.includes('duplicate key value'))return 'Ese cupo acaba de ser reservado por otra persona.';if(msg.includes('PAYMENT_HOLD_EXPIRED')||low.includes('payment_hold_expired'))return 'El tiempo para pagar el abono venció. La hora volvió a quedar disponible.';if(low.includes('mercadopago_not_configured'))return 'Los abonos todavía no están habilitados por Fila Cero. Intenta reservar otra hora o contacta al centro.';if(low.includes('mercadopago_preference_failed'))return 'Mercado Pago no pudo iniciar el cobro. La hora será liberada para que puedas intentarlo de nuevo.';if(msg.includes('Invalid login credentials'))return 'Correo o contraseña incorrectos.';if(low.includes('email rate limit exceeded'))return 'No pudimos enviar el correo de confirmación en este momento. Intenta más tarde o continúa con Google.';if(low.includes('unsupported provider')||low.includes('provider is not enabled'))return 'Google aún no está activado en Supabase. Habilita Authentication → Sign In / Providers → Google.';if(msg.includes('RESERVATION_NOT_OWNED'))return 'No tienes permiso para gestionar esa reserva.';if(msg.includes('FILA_CERO_ACCOUNT_BLOCKED'))return 'Esta cuenta está bloqueada dentro de Fila Cero por moderación.';if(msg.includes('FILA_CERO_ADMIN_REQUIRED'))return 'Esta sección es exclusiva del administrador de Fila Cero.';if(msg.includes('CANNOT_BLOCK_FILA_CERO_ADMIN'))return 'No puedes bloquear una cuenta administradora de Fila Cero.';if(msg.includes('REVIEW_TOO_EARLY'))return 'Podrás calificar después de que comience tu hora.';if(msg.includes('REVIEW_NOT_ALLOWED'))return 'Solo puedes calificar reservas asociadas a tu cuenta.';return msg}
...
function initSiteFooter(){
  ...
  const developerWeb='https://dev-web-ccp.vercel.app/';
  let footer=document.querySelector('footer.site-footer, footer[data-fc-footer]');
  if(!footer){
    footer=document.createElement('footer');
    footer.className='site-footer';
    footer.dataset.fcFooter='true';
    footer.innerHTML=`<div class="site-footer-brand"><span class="brand-mark">0</span><span><strong>FILA CERO</strong><small>Gran Concepción</small></span></div><div class="site-footer-copy"><span>Disponibilidad de último minuto, en un solo lugar.</span><small>Fila Cero no almacena fichas clínicas.</small></div><div class="developer-signature"><span class="developer-signature-label">DISEÑO Y DESARROLLO</span><a class="developer-credit developer-credit-web" href="${developerWeb}" target="_blank" rel="noopener noreferrer"><span class="developer-dot">●</span><span>Hecho por <strong>Dev Web CCP</strong></span><span aria-hidden="true">↗</span></a><a class="developer-instagram" href="${instagram}" target="_blank" rel="noopener noreferrer">@dev_web_ccp ↗</a></div>`;
  ...
  if(!footer.querySelector('.developer-credit')){
    ...
    credit.href=developerWeb;
    credit.target='_blank';
    credit.rel='noopener noreferrer';
    credit.setAttribute('aria-label','Sitio web de Dev Web CCP');
...
async function initMarketplace(){
  ...
  async function loadSlots(){
    ...
    const slotPromise=sb.from(T_SLOTS).select('id,business_id,service,category,city,sector,address,slot_date,start_time,duration_minutes,normal_price,fila_price,requires_deposit,deposit_amount,status,created_at').eq('status','active').gte('slot_date',TODAY_STR).order('slot_date',{ascending:true}).order('start_time',{ascending:true});
    const businessPromise=sb.from(T_BUSINESSES).select('id,name,category,description,city,sector,address,whatsapp,instagram,website,portfolio_urls,latitude,longitude,is_active,profile_enabled,is_verified,views_count,created_at').eq('is_active',true).order('created_at',{ascending:false}).limit(100);
  ...
  function renderBusinessDirectory(){
    ...
    businessesGrid.innerHTML=visibleBusinesses.map(b=>{
      ...
      return `<article class="business-directory-card" data-business-id="${escapeHtml(b.id)}"><a class="business-directory-cover" href="empresa.html?id=${encodeURIComponent(b.id)}">${image}<span class="business-directory-category">${escapeHtml(b.category||'Profesional')}</span></a><div class="business-directory-body"><div><h3>${escapeHtml(b.name)} ${b.is_verified?'<span class="verified-badge" title="Profesional verificado">✓ Verificado</span>':''}</h3><p class="business-directory-location">⌖ ${escapeHtml(b.sector?b.sector+' · ':'')}${escapeHtml(b.city||'Gran Concepción')}</p></div><p class="business-directory-description">${escapeHtml((b.description||'Perfil profesional en Fila Cero.').slice(0,150))}</p><div class="business-directory-meta"><span><strong>${count}</strong> ${count===1?'cupo activo':'cupos activos'}</span>${wa?`<a href="https://wa.me/${wa}" target="_blank" rel="noopener">WhatsApp ↗</a>`:''}</div><a class="btn btn-dark btn-full" href="empresa.html?id=${encodeURIComponent(b.id)}">Ver perfil y disponibilidad <span>→</span></a></div></article>`
    }).join('');
  }
  ...
  function render(){
    ...
    grid.innerHTML=items.map(s=>{const savings=s.normalPrice>0?Math.max(0,Math.round((1-s.price/s.normalPrice)*100)):0;return `<article class="slot-card" data-business-id="${escapeHtml(s.businessId)}"><div class="slot-top"><div class="service-icon">${ICONS[s.category]||'⚡'}</div><span class="badge-live">● DISPONIBLE</span></div><div><h3>${escapeHtml(s.service)}</h3>${s.business?.profile_enabled===false?`<span class="provider">${escapeHtml(s.provider)}</span><span class="profile-private-chip">Perfil no público</span>`:`<a class="provider provider-link" href="${businessLink(s)}">${escapeHtml(s.provider)}${s.business?.is_verified?' <span class="verified-inline">✓</span>':''} ↗</a>`}</div><div class="meta-list"><div class="meta-row">◷ <span>${formatDate(s.date)} · <strong>${escapeHtml(s.time)}</strong> · ${Number(s.duration)} min</span></div><div class="meta-row">⌖ <span>${escapeHtml(s.address)}${s.sector?` · ${escapeHtml(s.sector)}`:''}<br><b>${escapeHtml(s.city)}</b></span></div></div><div class="location-actions"><button class="map-btn show-map-btn" type="button" data-id="${escapeHtml(s.id)}">⌖ Ver mapa</button><a class="map-btn" href="${buildMapsSearchUrl(s)}" target="_blank" rel="noopener">Cómo llegar ↗</a></div><div class="slot-bottom"><div><div class="price-old">${money(s.normalPrice)}</div><div class="price-new">${money(s.price)}</div>${s.requiresDeposit?`<div class="deposit-chip">Abono ${money(s.depositAmount)}</div>`:''}${savings?`<div class="savings">${savings}% menos que el valor normal</div>`:''}</div><button class="btn btn-accent reserve-btn" data-id="${escapeHtml(s.id)}">Reservar <span>→</span></button></div></article>`}).join('');
  ...
  function openBooking(id){
    ...
    const bookingInfo=document.getElementById('bookingBusinessInfo');bookingInfo.innerHTML=`<div class="booking-business-head"><div class="booking-business-avatar">${escapeHtml((b.name||'FC').split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</div><div><span>TE ATIENDE</span><a href="${businessLink(selectedSlot)}">${escapeHtml(selectedSlot.provider)}${b.is_verified?' <span class="verified-inline">✓</span>':''} ↗</a><small>${escapeHtml(b.category||selectedSlot.category)} · ${escapeHtml(b.sector?b.sector+' · ':'')}${escapeHtml(b.city||selectedSlot.city)}</small></div></div>${b.description?`<p>${escapeHtml(b.description)}</p>`:''}${portfolio.length?`<div class="booking-mini-portfolio">${portfolio.slice(0,6).map((url,i)=>`<button class="booking-mini-photo" type="button" data-portfolio-index="${i}" aria-label="Ver imagen ${i+1}"><img src="${escapeHtml(url)}" alt="${escapeHtml(selectedSlot.provider)} ${i+1}" loading="lazy"></button>`).join('')}</div>`:''}${links.length?`<div class="booking-business-links">${links.join('')}</div>`:''}`;bindPortfolioViewer(bookingInfo,portfolio,selectedSlot.provider);
    renderLocationMap(document.getElementById('bookingMap'),selectedSlot);modal.classList.remove('hidden');sb.auth.getSession().then(({data})=>{const u=data?.session?.user;if(!u)return;if(!clientEmail.value)clientEmail.value=u.email||'';if(!clientName.value)clientName.value=u.user_metadata?.fila_cero_display_name||u.user_metadata?.full_name||u.user_metadata?.name||''}).catch(()=>{})
  }
  ...
  function reservationCalendarData(slot){return {service:slot.service,businessName:slot.provider,date:slot.date,time:slot.time,duration:slot.duration,location:fullLocation(slot),price:money(slot.price),timeZone:'America/Santiago'}}
  function saveLocalReceipt(receipt){try{const current=JSON.parse(localStorage.getItem('fc_my_reservations')||'[]');if(!current.some(x=>x.id===receipt.id))current.push(receipt);localStorage.setItem('fc_my_reservations',JSON.stringify(current))}catch{}}
  function showBookingSuccess(receipt){
    const success=document.getElementById('bookingSuccessModal');if(!success)return;const s=receipt.slot,cal=reservationCalendarData(s);document.getElementById('bookingSuccessSummary').innerHTML=`<strong>${escapeHtml(s.service)}</strong><span>${escapeHtml(s.provider)}</span><small>${formatDate(s.date)} · ${escapeHtml(s.time)} · ${Number(s.duration)} min</small><small>⌖ ${escapeHtml(fullLocation(s))}</small>`;const google=document.getElementById('bookingGoogleCalendar');google.href=window.FCCALENDAR?.googleUrl(cal)||'#';document.getElementById('bookingDownloadIcs').onclick=()=>window.FCCALENDAR?.downloadIcs(cal);success.classList.remove('hidden');
  }
  const successModal=document.getElementById('bookingSuccessModal');document.getElementById('closeBookingSuccess')?.addEventListener('click',()=>successModal?.classList.add('hidden'));document.getElementById('continueMarketplace')?.addEventListener('click',()=>successModal?.classList.add('hidden'));successModal?.addEventListener('click',e=>{if(e.target===successModal)successModal.classList.add('hidden')});
  document.getElementById('bookingForm').addEventListener('submit',async e=>{
    e.preventDefault();if(!selectedSlot)return;
    const button=e.submitter||e.currentTarget.querySelector('button[type="submit"]');button.disabled=true;button.textContent=selectedSlot.requiresDeposit?'Preparando abono…':'Reservando…';
    try{
      const params={p_slot_id:selectedSlot.id,p_client_name:clientName.value.trim(),p_client_email:clientEmail.value.trim(),p_client_phone:clientPhone.value.trim(),p_reminder_email:document.getElementById('reminderEmail')?.checked!==false,p_reminder_whatsapp:document.getElementById('reminderWhatsapp')?.checked===true,p_reminder_push:document.getElementById('reminderPush')?.checked!==false};
      const {data,error}=await sb.rpc(RPC_START_BOOKING,params);if(error)throw error;const row=Array.isArray(data)?data[0]:data;if(!row?.reservation_id)throw new Error('No se pudo crear la reserva.');
      const receipt={id:row.reservation_id,slot:{...selectedSlot},clientName:clientName.value.trim(),createdAt:new Date().toISOString(),status:row.reservation_status||'confirmed'};
      if(row.payment_required){
        showToast(`Hora retenida por 15 minutos. Preparando abono de ${money(row.payment_amount)}…`);
        const {data:pay,error:payError}=await sb.functions.invoke(EDGE_CREATE_PAYMENT,{body:{reservationId:row.reservation_id,accessToken:row.payment_access_token}});if(payError)throw payError;if(!pay?.checkoutUrl)throw new Error(pay?.error||'No se pudo abrir Mercado Pago.');
        localStorage.setItem('fc_pending_payment',JSON.stringify({...receipt,accessToken:row.payment_access_token,paymentExpiresAt:row.payment_expires_at}));location.href=pay.checkoutUrl;return;
      }
      saveLocalReceipt({...receipt,status:'confirmed'});e.target.reset();document.getElementById('reminderEmail').checked=true;document.getElementById('reminderPush').checked=true;modal.classList.add('hidden');showBookingSuccess({...receipt,status:'confirmed'});showToast('¡Reserva confirmada! Puedes agregarla a tu calendario.');await loadSlots();
...
async function initProfessional(){
  ...
  const today=TODAY_STR;slotDate.min=today;slotDate.value=today;const requiresDepositEl=document.getElementById('requiresDeposit'),depositAmountEl=document.getElementById('depositAmount'),depositAmountWrap=document.getElementById('depositAmountWrap');const syncDeposit=()=>{const enabled=requiresDepositEl?.checked===true;if(depositAmountEl)depositAmountEl.disabled=!enabled;depositAmountWrap?.classList.toggle('disabled',!enabled)};requiresDepositEl?.addEventListener('change',syncDeposit);syncDeposit();
  ...
  function renderReservationsForBusiness(){
    ...
    box.innerHTML=reservations.map(r=>{const s=slots.find(x=>x.id===r.slot_id);const phone=safeDigits(r.client_phone);const created=r.created_at?new Intl.DateTimeFormat('es-CL',{dateStyle:'medium',timeStyle:'short'}).format(new Date(r.created_at)):'';const confirmed=r.status==='confirmed',pending=r.status==='pending_payment';return `<article class="booking-row booking-row-rich"><div class="booking-client"><span class="client-avatar">${escapeHtml((r.client_name||'?')[0])}</span><div><strong>${escapeHtml(r.client_name)}</strong><small>${escapeHtml(r.client_email)}</small><small>${escapeHtml(r.client_phone)}</small><div class="booking-client-actions">${phone?`<a href="https://wa.me/${phone}" target="_blank" rel="noopener">WhatsApp ↗</a>`:''}<a href="mailto:${encodeURIComponent(r.client_email||'')}">Correo ↗</a></div></div></div><div class="booking-service-detail"><span class="eyebrow">${confirmed?'CITA RESERVADA':pending?'ABONO PENDIENTE':'CITA CANCELADA'}</span><strong>${escapeHtml(s?.service||'Cupo reservado')}</strong><small>${s?.date?formatDate(s.date):''} · ${escapeHtml(String(s?.time||''))} · ${Number(s?.duration||0)} min</small><small>⌖ ${escapeHtml(s?fullLocation(s):business.address||'')}</small><small>${created?`Reservada: ${escapeHtml(created)}`:''}</small></div><div class="booking-row-side"><strong>${s?money(s.price):''}</strong>${pending?`<small class="pending-label">Abono ${money(r.payment_amount||0)} pendiente</small>`:r.payment_status==='paid'?'<small class="paid-label">✓ Abono pagado</small>':''}<span class="reservation-status">${escapeHtml(String(r.status||'confirmed').toUpperCase())}</span><div class="management-actions">${confirmed?`<button class="action-btn cancel-reservation-btn" type="button" data-id="${escapeHtml(r.id)}">Cancelar cita</button>`:''}<button class="action-btn danger delete-reservation-btn" type="button" data-id="${escapeHtml(r.id)}">Eliminar cita</button></div></div></article>`}).join('');
  ...
  function renderPro(){
    ...
    if(!slots.length)professionalSlots.innerHTML='<p class="muted-small">Todavía no has publicado cupos.</p>';else professionalSlots.innerHTML=slots.map(s=>`<div class="pro-slot"><strong>${escapeHtml(s.service)}</strong><small>${formatDate(s.date)} · ${escapeHtml(s.time)} · ${escapeHtml(s.city)}<br>${money(s.price)}${s.requiresDeposit?` · Abono ${money(s.depositAmount)}`:''}</small><div class="pro-slot-actions"><span class="reservation-status">${s.status==='reserved'?'RESERVADO':s.status==='cancelled'?'CANCELADO':isExpired(s)?'VENCIDO':'ACTIVO'}</span><div class="management-actions">${s.status==='active'?`<button class="action-btn cancel-slot-btn" type="button" data-id="${escapeHtml(s.id)}">Cancelar cupo</button>`:''}<button class="action-btn danger delete-slot-btn" type="button" data-id="${escapeHtml(s.id)}">Eliminar</button></div></div></div>`).join('');
  ...
  form.addEventListener('submit',async e=>{
    ...
    const requiresDeposit=requiresDepositEl?.checked===true,depositAmount=requiresDeposit?Number(depositAmountEl?.value||0):0;const payload={business_id:business.id,service:serviceName.value.trim(),category:category.value,city:business.city,sector:business.sector||'',address:business.address,slot_date:slotDate.value,start_time:slotTime.value,duration_minutes:Number(duration.value),normal_price:Number(normalPrice.value),fila_price:Number(filaPrice.value),requires_deposit:requiresDeposit,deposit_amount:depositAmount,status:'active'};
    if(payload.fila_price>payload.normal_price)return showToast('El precio Fila Cero no puede superar el precio normal.');if(payload.requires_deposit&&(payload.deposit_amount<=0||payload.deposit_amount>payload.fila_price))return showToast('El abono debe ser mayor a $0 y no puede superar el precio Fila Cero.');
...
async function initBusinessProfile(){
  ...
  function renderBusiness(){businessName.innerHTML=`${escapeHtml(business.name)}${business.is_verified?' <span class="verified-badge verified-badge-light">✓ Verificado</span>':''}`;businessCategoryBadge.textContent=(business.category||'CENTRO PROFESIONAL').toUpperCase();businessLocation.textContent=`${business.sector?business.sector+' · ':''}${business.city}`;businessDescription.textContent=business.description||'Este centro todavía no ha agregado una descripción pública.';businessAvatar.textContent=(business.name||'FC').split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase();asideBusinessName.textContent=business.name;asideAddress.textContent=`${business.address||'Dirección por completar'}${business.sector?`, ${business.sector}`:''}, ${business.city}`;
...
async function initAdmin(){
  ...
  async function loadAdmin(){
    try{
      ...
      businesses=businessRows||[];window.__fcAdminBusinesses=businesses;blocked=blockedRows||[];renderBusinesses();renderBlocked();
