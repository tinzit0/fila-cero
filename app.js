...
async function initProfessional(){
  ...
  businessProfileForm.addEventListener('submit',async e=>{
    ...
    try{
      ...
      const profileDraft={name:profileName.value.trim(),category:profileCategory.value,description:profileDescription.value.trim(),city:profileCity.value,sector:profileSector.value.trim(),address:profileAddress.value.trim(),whatsapp:safeDigits(profileWhatsapp.value),instagram:profileInstagram.value.replace(/^@/,'').trim(),website:safeHttpUrl(profileWebsite.value),portfolio_urls:portfolio};
      ...
      publicProfileLink.href=`empresa.html?id=${encodeURIComponent(business.id)}`;profilePreviewButton.href=publicProfileLink.href;if(fileInput)fileInput.value='';portfolioState=business.portfolio||[];loadProfile();showToast('Perfil público guardado en Supabase.');await loadDashboard()
