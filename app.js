...
async function initMarketplace(){
  ...
  async function loadSlots(){
    ...
    const {data,error}=await sb.from('slots').select(`id,business_id,service,category,city,sector,address,slot_date,start_time,duration_minutes,normal_price,fila_price,status,created_at,businesses(id,name,category,description,whatsapp,instagram,website,portfolio_urls,is_active)`).eq('status','active').gte('slot_date',TODAY_STR).order('slot_date',{ascending:true}).order('start_time',{ascending:true});
    loading=false;
    if(error){console.error(error);resultsText.textContent='No pudimos cargar los cupos. Revisa que hayas ejecutado el SQL de Supabase.';return}
    slots=(data||[]).map(normalizeSlot).filter(s=>SUPPORTED_COMMUNES.includes(s.city)&&!isExpired(s));
  ...
  document.getElementById('bookingForm').addEventListener('submit',async e=>{
    ...
    try{
      const {data,error}=await sb.rpc('book_slot',{p_slot_id:selectedSlot.id,p_client_name:clientName.value.trim(),p_client_email:clientEmail.value.trim(),p_client_phone:clientPhone.value.trim()});
  ...
  const channel=sb.channel('fila-cero-marketplace').on('postgres_changes',{event:'*',schema:'public',table:'slots'},()=>loadSlots()).on('postgres_changes',{event:'UPDATE',schema:'public',table:'businesses'},()=>loadSlots()).subscribe();
...
async function initProfessional(){
  ...
  function mapOwnedSlot(row){return normalizeSlot({...row,businesses:business})}
  ...
  async function loadDashboard(){
    const slotReq=await sb.from('slots').select('*').eq('business_id',business.id).order('created_at',{ascending:false});
    if(slotReq.error)throw slotReq.error;
    slots=(slotReq.data||[]).map(mapOwnedSlot);
    const resReq=await sb.from('reservations').select('id,slot_id,business_id,client_name,client_email,client_phone,status,created_at,slots(service,slot_date,start_time,fila_price)').eq('business_id',business.id).order('created_at',{ascending:false});
  ...
  function renderReservationsForBusiness(){const box=businessReservations;if(!reservations.length){box.innerHTML='<div class="dashboard-empty"><strong>Aún no hay horas reservadas.</strong><span>Cuando una persona tome uno de tus cupos, aparecerá aquí con sus datos de contacto.</span></div>';return}box.innerHTML=reservations.map(r=>{const s=Array.isArray(r.slots)?r.slots[0]:r.slots;return `<article class="booking-row"><div class="booking-client"><span class="client-avatar">${escapeHtml((r.client_name||'?')[0])}</span><div><strong>${escapeHtml(r.client_name)}</strong><small>${escapeHtml(r.client_email)} · ${escapeHtml(r.client_phone)}</small></div></div><div><strong>${escapeHtml(s?.service||'Cupo reservado')}</strong><small>${s?.slot_date?formatDate(s.slot_date):''} · ${escapeHtml(String(s?.start_time||'').slice(0,5))}</small></div><div><span class="reservation-status">${escapeHtml(String(r.status||'confirmed').toUpperCase())}</span></div></article>`}).join('')}
  function renderPro(){const active=slots.filter(s=>s.status==='active'&&!isExpired(s)),booked=slots.filter(s=>s.status==='reserved');activeSlotsCount.textContent=active.length;bookedSlotsCount.textContent=reservations.filter(r=>r.status==='confirmed').length;if(!slots.length)professionalSlots.innerHTML='<p class="muted-small">Todavía no has publicado cupos.</p>';else professionalSlots.innerHTML=slots.map(s=>`<div class="pro-slot"><strong>${escapeHtml(s.service)}</strong><small>${formatDate(s.date)} · ${escapeHtml(s.time)} · ${escapeHtml(s.city)}<br>${money(s.price)}</small><div class="pro-slot-actions"><span class="reservation-status">${s.status==='reserved'?'RESERVADO':s.status==='cancelled'?'CANCELADO':isExpired(s)?'VENCIDO':'ACTIVO'}</span>${s.status==='active'?`<button class="delete-btn" data-id="${escapeHtml(s.id)}">Cancelar cupo</button>`:''}</div></div>`).join('');professionalSlots.querySelectorAll('.delete-btn').forEach(btn=>btn.addEventListener('click',async()=>{const {error}=await sb.from('slots').update({status:'cancelled'}).eq('id',btn.dataset.id).eq('business_id',business.id);if(error)return showToast(humanError(error));showToast('Cupo cancelado.');await loadDashboard()}));renderLivePreview();renderReservationsForBusiness()}
  ...
  businessProfileForm.addEventListener('submit',async e=>{
    ...
    try{
      ...
      await sb.from('slots').update({city:business.city,sector:business.sector||'',address:business.address}).eq('business_id',business.id).eq('status','active');
  ...
  const channel=sb.channel(`fila-cero-business-${business.id}`).on('postgres_changes',{event:'*',schema:'public',table:'slots',filter:`business_id=eq.${business.id}`},()=>loadDashboard()).on('postgres_changes',{event:'*',schema:'public',table:'reservations',filter:`business_id=eq.${business.id}`},()=>loadDashboard()).on('postgres_changes',{event:'UPDATE',schema:'public',table:'businesses',filter:`id=eq.${business.id}`},async()=>{business=await FCAUTH.currentBusiness();loadProfile()}).subscribe();
  window.addEventListener('beforeunload',()=>{try{sb.removeChannel(channel)}catch{}});
...
async function initBusinessProfile(){
  ...
  async function loadPublicSlots(){const {data,error}=await sb.from('slots').select('*').eq('business_id',business.id).eq('status','active').gte('slot_date',TODAY_STR).order('slot_date',{ascending:true}).order('start_time',{ascending:true});if(error)throw error;return (data||[]).map(r=>normalizeSlot({...r,businesses:business})).filter(s=>!isExpired(s))}
  ...
  const channel=sb.channel(`fila-cero-profile-${business.id}`).on('postgres_changes',{event:'*',schema:'public',table:'slots',filter:`business_id=eq.${business.id}`},renderSlots).on('postgres_changes',{event:'UPDATE',schema:'public',table:'businesses',filter:`id=eq.${business.id}`},async()=>{business=await FCAUTH.getBusiness(business.id);renderBusiness()}).subscribe();
  window.addEventListener('beforeunload',()=>{try{sb.removeChannel(channel)}catch{}})
}
