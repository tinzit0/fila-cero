(function(){
  ...
  async function initAccount(){
    ...
    async function loadHistory(){
      ...
      const slotIds=[...new Set(rs.map(r=>r.slot_id))];const {data:slots}=await sb.from(T_SLOTS).select('*').in('id',slotIds);const slotMap=new Map((slots||[]).map(x=>[x.id,x]));const businessIds=[...new Set(rs.map(r=>r.business_id))];const {data:businesses}=await sb.from(T_BUSINESSES).select('id,name,is_verified,address,sector,city').in('id',businessIds);const bm=new Map((businesses||[]).map(x=>[x.id,x]));const {data:reviews}=await sb.from(T_REVIEWS).select('reservation_id,rating,comment').eq('user_id',user.id);const rm=new Map((reviews||[]).map(x=>[x.reservation_id,x]));const calendarItems=new Map();
      $('#accountReservations').innerHTML=rs.map(r=>{const sl=slotMap.get(r.slot_id),b=bm.get(r.business_id),review=rm.get(r.id),past=sl?new Date(`${sl.slot_date}T${String(sl.start_time).slice(0,5)}`)<=new Date():false,confirmed=r.status==='confirmed';let cal=null;if(sl&&b){cal={service:sl.service,businessName:b.name,date:sl.slot_date,time:String(sl.start_time).slice(0,5),duration:Number(sl.duration_minutes||30),location:`${sl.address||b.address||''}${sl.sector||b.sector?`, ${sl.sector||b.sector}`:''}, ${sl.city||b.city||'Gran Concepción'}, Chile`,price:money(sl.fila_price),timeZone:'America/Santiago'};calendarItems.set(r.id,cal)}return `<article class="account-reservation-card"><div><span class="reservation-status">${esc(String(r.status).toUpperCase())}</span><h3>${esc(sl?.service||'Reserva')}</h3><p>${esc(b?.name||'Centro')} ${b?.is_verified?'<span class="verified-inline">✓</span>':''}</p><small>${sl?`${date(sl.slot_date)} · ${esc(String(sl.start_time).slice(0,5))} · ${esc(sl.city)}`:''}</small>${r.payment_status==='paid'?'<small class="paid-label">✓ Abono pagado</small>':r.payment_status==='pending'?'<small class="pending-label">Abono pendiente</small>':''}</div><div class="account-reservation-actions">${b?`<a class="action-btn" href="empresa.html?id=${encodeURIComponent(b.id)}">Ver perfil</a>`:''}${confirmed&&cal?`<a class="action-btn" href="${esc(window.FCCALENDAR?.googleUrl(cal)||'#')}" target="_blank" rel="noopener">Google Calendar</a><button class="action-btn ics-reservation-btn" data-id="${esc(r.id)}">.ics</button>`:''}${past&&r.status!=='cancelled'?`<button class="action-btn review-reservation-btn" data-id="${esc(r.id)}" data-rating="${review?.rating||''}" data-comment="${esc(review?.comment||'')}">${review?'Editar reseña':'Calificar'}</button>`:''}</div></article>`}).join('');
      $$('.review-reservation-btn').forEach(btn=>btn.onclick=()=>openReview(btn.dataset.id,btn.dataset.rating,btn.dataset.comment));$$('.ics-reservation-btn').forEach(btn=>btn.onclick=()=>{const cal=calendarItems.get(btn.dataset.id);if(cal)window.FCCALENDAR?.downloadIcs(cal)});
    }
    ...
    $('#enableBrowserNotifications').onclick=async()=>{try{if(!window.FCPWA)throw new Error('La función push aún no cargó.');await window.FCPWA.subscribePush()}catch(e){toast(e.message||String(e))}};
    const ch=sb.channel(`fc-notifications-${user.id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:T_NOTIFICATIONS,filter:`user_id=eq.${user.id}`},payload=>{loadNotifications()}).subscribe();window.addEventListener('beforeunload',()=>sb.removeChannel(ch));
  ...
  async function initGlobalNotifications(){
    ...
    await refresh();
    if(!$('#userAccountPage')){const ch=sb.channel(`fc-global-notifications-${s.user.id}`).on('postgres_changes',{event:'INSERT',schema:'public',table:T_NOTIFICATIONS,filter:`user_id=eq.${s.user.id}`},payload=>{refresh()}).subscribe();window.addEventListener('beforeunload',()=>{try{sb.removeChannel(ch)}catch{}})}
  }
